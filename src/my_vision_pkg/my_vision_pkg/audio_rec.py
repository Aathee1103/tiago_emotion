...
class VoiceToSpeech(Node):
    ...
    def __init__(self):
        ...
        self.voice_service = self.create_service(
            Trigger,
            '/start_voice',
            self.start_voice_callback)

    def start_voice_callback(self, request, response):

        if not self.active:
            self.active = True

            self.get_logger().info("🎤 Voice interaction started")

            self.send_speak_once()

        response.success = True
        response.message = "Voice node started"
